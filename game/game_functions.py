...
def check_keydown_evnet(event, ai_settings, screen, ship, bullets):
...
def check_event(ai_settings, screen, ship, bullets, play_button, stats, aliens, sb):
    for event in pygame.event.get():
        if (event.type == pygame.QUIT):
            sys.exit()
        elif(event.type == pygame.KEYDOWN):
            check_keydown_evnet(event, ai_settings, screen, ship, bullets)
...
def update_game(ai_settings, screen, stats, ship, aliens, bullets, play_button, sb):
...
def get_number_aliens(ai_settings, alien_width):
    ...
    return int(avalilable_space_x / (2 * alien_width))
